...
[[ -f $artifact/figori && -f $artifact/Panel.qml && -f $artifact/manifest.json && -f $artifact/figori-mark.svg ]] || { echo 'Incomplete artifact' >&2; exit 2; }
...
install -m 755 "$artifact/figori" "$cli.new"
mv -T "$cli.new" "$cli"
mkdir "$backup/new-plugin"
cp "$artifact/manifest.json" "$artifact/Panel.qml" "$artifact/figori-mark.svg" "$backup/new-plugin/"
