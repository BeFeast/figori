...
Bare English month names (`nov`, `November`) mean day 1 in the selected anchor year, without next-year rollover. `november 2027` supplies an explicit year. The basis note states the assumed day and year; thus `days until november` is 45 calendar days on local today `2026-09-17`. An existing exact-name worksheet variable takes precedence over a bare month name, preserving assignments. Full and three-letter month names are case insensitive; arbitrary prefixes and `month`/`months` are not date names.
